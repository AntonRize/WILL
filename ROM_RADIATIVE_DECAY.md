...
- **A1 — Adiabatic representability** (assumption). The radiating binary is a sequence of closed R.O.M. states (beta(N), e(N)) with per-cycle drifts |Delta ln beta²|, |Delta e| ≪ 1. The closed set classifies exact states only; representing an open system as a drifting sequence of closed states is an approximation the closed set does not itself supply. It is self-consistent wherever the resulting fluxes are small, and fails near marginal stability (see Step 5).
- **A2 — Scale constancy** (assumption, with stated error). dR_s/dN = 0 at leading order. The radiated ledger also drains the system scale, but by ΔlnR_s ~ eta (beta²/2) epsilon_W per cycle — suppressed by eta beta² relative to the orbital drift epsilon_W. Neglected terms are O(eta beta²) of the terms kept.
...
- **A4 — Channel universality** (assumption; used only in §7). The fluxes are universal functions of the extended state: epsilon_W = F_W(beta, e, eta), epsilon_h = F_h(beta, e, eta).
- **A5 — Two-scale identification** (forced by consistency of the extension, not by the closed set; used only in §7/§9). For comparable scales the closed set's R_s must be read as R_s1 + R_s2, otherwise the Kepler identity T = 2 pi a/(beta c) with beta² = R_s/(2a) fails for the relative orbit.
...
- From a = R_s/(2 beta²):  **d ln a/dN = −epsilon_W**
- From T = pi R_s/(beta³ c):  **d ln T/dN = −(3/2) epsilon_W**, hence the dimensionless period drift **Tdot = −(3/2) epsilon_W**
- From h_W = R_s c sqrt(1−e²)/(2 beta):  **de/dN = ((1−e²)/e) (epsilon_h − epsilon_W/2)**
- From Delta_phi = 2 pi (3 beta² − 2 beta⁴)/(1−e²) by chain rule:  **d(Delta_phi)/dN = (4 pi beta²/(1−e²)) (3 epsilon_h − beta² epsilon_W − 2 beta² epsilon_h)**
...
- per-cycle radiated ledger, per unit E_0:  **−dE/dN = (beta² epsilon_W/2) · (1−beta²)^(−3/2) (1−2 beta²)^(−1/2)**
- weak field: −dE/dN → (beta²/2) epsilon_W · (1 + (5/2) beta² + ...), recovering the Newtonian binding-energy rate with the classical 1/2 as the first Taylor coefficient, exactly as WILL_RG_I derives the classical limit.
- The Energy Symmetry Law (Delta E_A→B + Delta E_B→A = 0) forces this drained ledger to appear in the open channel — bookkeeping balance, not a rate.
...
- **C1 (Mathematical Transparency; zero free parameters).** The fluxes are dimensionless functions of dimensionless state: F_W(beta, e, eta), F_h(beta, e, eta). Any dimensionful radiated power may use G and c only as translation factors; the unique luminosity scale so constructed is c⁵/G, hence P = (c⁵/G) · p(beta, e, eta) with p dimensionless.
- **C2 (Relational Reciprocity).** Q_A→B = Q_B→A suggests symmetry under R_s1 ↔ R_s2: dependence on the scales only through symmetric ratios; minimal choice eta.
- **C3 (Circularity lock — forced by the web, §4 Step 6).** F_h(beta, 0, eta) = F_W(beta, 0, eta)/2.
- **C4 (Closure Condition).** F → 0 in the exactly closed limit: a system with no open channel does not leak. In particular F must vanish as the two-body relation is switched off.
- **C5 (Domain).** Defined on 0 < beta² < 1/6, e in [0,1), 0 < eta ≤ 1/4.
...
- Part B (4): time-averaged closure over a conservative cycle — ⟨kappa_o²⟩_t = 2 beta², ⟨beta_o²⟩_t = beta², defect zero; base Kepler integrals at 30 digits for e = 0.1–0.9.
- Part C (4): each line of the kinematic web is an exact identity (the de/dN and dDelta_phi/dN forms were corrected *by* this check during derivation — the initial hand-derived sign of de/dN was wrong; the script caught it).
...
- GR (Schwarzschild): 1 − x/2 **+ (3/8) x²** + ...
- RG (kappa_X/beta_Y): 1 − x/2 **− (5/8) x²** + ...
