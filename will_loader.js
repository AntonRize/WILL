// Loader for WILL narrative sections — copy this as will\_loader.js and include in your interactive.html
// Requires: narrative/will\_narrative1.txt in UTF-8, mapping is hardcoded below or generated from mapping file

// 1. Map: section header text (as found in TXT) => html id (from your cleaned\_interactive.html)
const sectionMap = {
...
fetch('narrative/will\_narrative1.txt')
.then(r => r.text())
.then(text => {
// 3. Split into sections by header: match lines starting with \**, ##, or known patterns
// Change this regex to fit your TXT structure
const splitRegex = /^(\*\*|##)\s?(.+?)\s*\$/gm;
let sections = \[];
let lastIdx = 0;
let match;
let headers = \[];
...
// Optionally, you may want to support markdown via marked.js
// Replace innerText with innerHTML + marked(section.body) if desired