// src/App.jsx  –  RAW-FULL  (English-default)
...
/* 3 – Load whole KB as one string */
async function loadKB() {
  const base = 'https://raw.githubusercontent.com/AntonRize/WILL/main/';
  const txts = await Promise.all(FILES.map(p => fetch(base + p).then(r => r.text())));
  return txts.join('\n\n');
}
...
export default function App() {
  ...
  async function send() {
    ...
    setBusy(true);
    try {
      const langDirective = isRussian(q) ? 'Russian' : 'English';
      const prompt = `
You are the WILL Geometry assistant. Answer in **${langDirective}** only.
